...
def addNewReview(newCountP):
    if newCountP == 'positive':
        rating = round((positiveCount + 1) / (positiveCount + negativeCount + 1) * 10, 2)
    else:
        rating = round(positiveCount / (positiveCount + negativeCount + 1) * 10, 2)
...
def checkN():
    if app.save_feedback:
        Y = addNewReview(app.save_feedback)
    else:
        Y = finalRating(0)
